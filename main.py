...
# We need the two next ones to create admin-only decorators
...
# We need to set up environment vars to replace SQLAlchemy with PostgreSQL
import os
...
# CONFIGURE DATABASE TABLE FOR BLOGPOSTS
...
def login():
    form = LoginForm()
    # If user POST the form
    if form.validate_on_submit():
        # Getting data from the form
        email = form.email.data
        password = form.password.data
        # Getting user from the database, based on email
        user = User.query.filter_by(email=email).first()
        # User with submitted email does not exist in database
        if not user:
